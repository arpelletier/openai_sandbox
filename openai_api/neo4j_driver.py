...
def testing():
    ...
    query = """MATCH (d:DrugBank_Compound)-[:`-treats->`]->(disease) WHERE disease.name IN ['Entrez:202333', 'MeSH_Tree_Disease:C23.550.513.355.750', 'MeSH_Disease:D002312'] MATCH (d)-[:`-drug_targets_protein->`]->(p:UniProt) MATCH (g:Entrez)-[:`-encodes->`]->(p) RETURN d.name AS drug_name, p.name AS protein_name, g.name AS gene_name"""
    print(query)
    driver = Driver()
    driver.verify_connectivity()
    result = driver.check_query(query)
    raw_results = random.sample(result[2], 30)
